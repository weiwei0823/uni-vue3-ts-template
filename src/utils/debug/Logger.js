...
var LogWatcher = function (logName) {
    ...
    this.save = function (code, response, desc = '') {
        ...
        let logger = config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_DEBUG_LOG_HTTP;
...
var UserInfoGetWatcher = function (logName) {
    this.seed = Date.now() + "" + Math.ceil(Math.random() * 99999);
    this.logName = logName;
    this.startTime = -1;
    this.endTime = -1;
    ...
    this.save = function (code, response, desc = '') {
        try {
            ...
            this.endTime = Date.now();
            let uInfo=response;
            let userInfoSort={ Uid:uInfo&&uInfo.id?uInfo.id:'null',
                name:uInfo&&uInfo.nickname?uInfo.nickname:"null" ,
                headImgUrl:uInfo&&uInfo.headImgUrl?uInfo&&uInfo.headImgUrl.slice(uInfo.headImgUrl.length-37,uInfo.headImgUrl.length):"null" }
            let responseStr = JSON.stringify(userInfoSort);
            let saveObject = {
                code: code,
                desc: desc,
                response: responseStr.substring(0, this.responseMaxSize),
            ...
            let logger = config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_DEBUG_LOG_USERINFO;
            let configMax = config.loggerMax;
            let logArray = uni.getStorageSync(logger);
            if (!logArray) logArray = [];
            if (logArray.length > configMax) logArray.pop();
...
export default {
    ...
    printWebsocketLotteryMessageLog(data) {
        ...
        let lotteryInfo = JSON.parse(data.msgTxt);
        ...
        console.log("stampType [当前下注回合状态]", lotteryInfo.stampType, "1 封盘段 2 购彩段 3封单(关盘)段");
        console.log("closeTimeStamp [当前下注回合剩余多少秒]", lotteryInfo.closeTimeStamp);

        console.log("=================================");
        console.log("[客户端模拟进入下一回合?]", lotteryInfo['nextRound'] != null);
        lotteryInfo['nextRound'] != null && console.log("[客户端模拟下一回合]", lotteryInfo['nextRound']['nextShorNo']);
        lotteryInfo['nextRound'] != null && console.log("[客户端模拟下一回合]", lotteryInfo['nextRound']["nextNo"]);
        lotteryInfo['nextRound'] != null && console.log("[客户端模拟进入下一回剩余时间]", lotteryInfo['nextRound']['closeTimeStamp']);
        lotteryInfo['nextRound'] != null && console.log("[客户端模拟下一回合信息]", lotteryInfo['nextRound']);
        //#endif
    }
