...
export default {
    ...
    writeStationIcoMsgLink(){
        let statioIcoContantsKey = config.enumMgr.CONTANTS_CONFIGS_ID
            .STATION_LOGO;
        let icoMsg =  $store.state.appInfoStore.contantsConfigTypeMap[statioIcoContantsKey];
        if(!icoMsg||!icoMsg.url){
            console.warn(`CONTATNS ${statioIcoContantsKey} is null. `)
            return;
        }
        let iconUrl = icoMsg.url;
     ...
     */
    openPointerBreaks(){
        let ctrl = getQueryString("closePointer");
