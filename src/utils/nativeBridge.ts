export default {
  ...
  openUrl(openUrl) {
    if (browserPlatform() == browserPlatform.FLUTTER_App) {
      window.flutter_inappwebview &&
        window.flutter_inappwebview?.callHandler('openExtraUrl', openUrl)
  ...
  setSafeAreaColor(color) {
    window.flutter_inappwebview?.callHandler('bottomSpaceColor', color)
  },
  /**
   * 调整APP顶部颜色
   * @param color
  ...
  callNativeGetHandler(eventName) {
    ...
    try {
      ...
      if (!window?.flutter_inappwebview?.callHandler) {
        const timmer = setInterval(() => {
          const _seed = _this.buildClientSeed()
          if (window?.flutter_inappwebview?.callHandler) {
      ...
      const seed = this.buildClientSeed()
  ...
  buildClientSeed() {
    const time = Date.now()
    const prefix = Math.ceil(Math.random() * 10000)
      .toString()
      .padStart(4, '0')
    const createSeed = `${time}${prefix}`
  ...
  doGetNativeMessage(event, seed) {
    ...
    if (!window.flutter_inappwebview)
      console.warn('flutter_inappwebview 对象错误')
    window.flutter_inappwebview &&
      window.flutter_inappwebview?.callHandler(event, seed)
  },
  /**
   *
   * @param eventName
  ...
  doGetNativeCallHandler(eventName, nativeCallBackName, callBack, region) {
    const seed = this.buildClientSeed()
    this.addOneNativeCallbackListener(
      eventName,
      nativeCallBackName,
      seed,
      true,
  ...
  doAwaitGetNativeCallHandler(eventName, nativeCallBackName, timeOut = 0) {
    ...
    const promise = new Promise((resolve) => {
      that.addOneNativeCallbackListener(
        eventName,
        nativeCallBackName,
        seed,
        true,
  ...
  addOneNativeCallbackListener(
    ...
    const eventPool = this.eventPools
    if (!eventPool[nativeCallBackName]) {
      ...
      global[nativeCallBackName] = function (res) {
        ...
        const runEvents = eventPool[nativeCallBackName]
        if (runEvents) {
          for (const seed in runEvents) {
            const eventItem = runEvents[seed]
            if (!eventItem._listenAll && targetSeed !== 1 && seed != targetSeed)
              continue
            if (eventItem._didNum < (eventItem._isOnece ? 1 : Infinity)) {
              ...
              setTimeout(() => {
                eventItem._cb &&
                  eventItem._reg &&
                  eventItem._cb.call(eventItem._reg, responseData)
              })
  ...
  on(
    ...
    region,
    isOnece = false,
    isClearOtherListener = false,
    eventFlg = 'onSetListener'
  ) {
    const eventPool = this.eventPools
