...
_g.pop = function (msg, time = 1000, icon = null, cb = null, cbRegion = null) {
    ...
    if (cb && cbRegion) args['complete'] = function () {
...
if (browserPlatform() == browserPlatform.IOS_App) {
    console.log = function (msg) {
        try {
            window.webkit.messageHandlers.jsListener.postMessage(JSON.stringify({event: "ON_CONSOLE_LOG", value: msg}));
...
_g.followKeybordHeight = function (pageInstance, windowTop = 0) {
    ...
    let oldPostion = pageInstance.$el.style.position;
    let oldTop = pageInstance.$el.style.top;
    var doChange = function (keyBoradOpend) {
        let systemInfo = uni.getSystemInfoSync();
        let winHeight = systemInfo.windowHeight;
        let offsetHeight = store.state.systemInfo.windowHeight - winHeight;
...
_g.buildSign = async function (slot, apikey) {
    return new Promise(resolve => {
        var tryTimes = 0;
        var run = function () {
            let model = window['SignModel'];
            if (model) {
                try {
                    var SignModel = window.SignModel;
                    var pr1 = SignModel.allocate(SignModel.intArrayFromString(slot), 0);
                    var pr2 = SignModel.allocate(SignModel.intArrayFromString(apikey), 0);
                    var pr3 = SignModel._getSignCode(pr1, pr2);
                    var sign = SignModel.UTF8ToString(pr3);
                    SignModel._free(pr1);
                    SignModel._free(pr2)
                    SignModel._free(pr3);
                    resolve(sign);
...
_g.printDebugLog = function (logMsg,level = printDebugLog.INFO,...others) {
    ...
    console.log(`----Print Log:${logMsg} 【${new Date().Format("yyyy-MM-dd HH:mm:ss")}】`,others);
}
