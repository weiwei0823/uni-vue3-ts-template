...
export const buyOrder = (prams) => {
	...
	let copyParms = Object.assign({}, prams);
	for (let k in copyParms.orderList) {
		let orderPice = copyParms.orderList[k];
		if (!orderPice) continue;
		let commitContent = orderPice['content_commit']
	...
	return new Promise(async (resolove) => {
		let result = await apiLoginRequest({
