...
const sokcet= (sockeUrl,onReceive,onErrorEvent,onErrorSucceed)=> {
    ...
    let value = config.apiKey +""+ salt;
    console.log(value, "-----", new Date().getTime());
    let secret = EncryptUtils.encryptMacToBase64(value, CryptoJS.MD5(CryptoJS.enc.Utf8.parse(config.apiKey)));
    let isDebug=false;
    //做两次转码处理 避免%2B被后端接收为空格
    let connectUrl=`${url}?salt=${salt}&secret=${encodeURIComponent(encodeURIComponent(secret))}`;
    ...
    socketTask.onOpen((res)=>{
        ...
        heartbeatInterval = setInterval(() => {
            ...
            if(lastSendHeartBeatTime-lastGetHeartBeatTime>beatInterval&&sendFirstHeartBeat){//大于三秒安全阈值 则表示心跳已断开
    ...
    socketTask.onMessage((res) => {
        ...
        let serverData = decryptStr(res.data);
...
const sendMsg = (msg)=>{   //向后端发送命令
    ...
    msg = EncryptUtils.encryptAES_StrToStr_test(JSON.stringify(msg), CryptoJS.MD5(CryptoJS.enc.Utf8
        .parse(config.apiKey)));
    try{
        //通过 WebSocket 连接发送数据
        socketTask.send({
