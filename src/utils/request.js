...
export const getApiTimeStamp = function (params) {
    ...
    //当前毫秒内是否被同API上锁
    if (!url || !apiTimestampDic[timestamp] || apiTimestampDic[timestamp] == url) {
 ...
 */
export const apiRequest = (prams, showErrorPage = false) => { //prams 为我们需要调用的接口API的参数 下面会贴具体代码
    // 判断请求类型
    ...
    let headerData = {
        ...
        'X-Access-Token': config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_TOKEN),
        'lotterytype': config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_LOTTERYTYPE) || 1,
...
export const apiRequestBase64 = (prams, showErrorPage = false) => {
    ...
    if (isShowLog) console.log("\n <---------加密通信开始doHttpRequestBase64 apikey=" + config.apiKey);
    let token = config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_TOKEN);
    ...
    headerData.secret = EncryptUtils.encryptMacToBase64(value, CryptoJS.MD5(CryptoJS.enc.Utf8.parse(config
        .apiKey)));
    console.log(value, "1111111")
    console.log(headerData.secret, "2222222")
    headerData = Object.assign(headerData, prams.header);
    isShowLog && console.log("【header】处理完成后结果=", JSON.stringify(headerData));

    let encryptData = {};
    if (prams.query && Object.keys(prams.query).length > 0) {
        console.log(prams.query)
        let data = EncryptUtils.encryptAES_StrToStr_test(JSON.stringify(prams.query), CryptoJS.MD5(CryptoJS.enc.Utf8
            .parse(config.apiKey)));
        encryptData = {
            encryptData: data
        };
    }
    isShowLog && console.log("【query】处理完成后结果=", JSON.stringify(encryptData));
    dataObj = encryptData;
    isShowLog && console.log("加密通信结束-------------------------------------->\n\n")
    j = 1;
...
export const apiLoginRequest = (prams) => {
    ...
    let token = config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_TOKEN);
    let timeSend = getApiTimeStamp(prams); //Math.round(new Date().getTime());
    let userInfo = config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_USERINFO);
...
async function resquest(prams, dataObj, headerData, trytimes = 0, showErrorPage = false) {
    ajaxTimes++;
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (cacheResult.status === 1 && cacheResult.data) {
            printDebugLog(`${prams.url} 接口 击中本地缓存 是否为强制请求后更新数据:${!!cacheResult.fromForceUpdateCache}`);
            ...
            setTimeout( ()=>{
                if(!cacheResult.data[config.enumMgr.API_CACHE_CALLBACK_NAME]){
                    console.error(`api => ${url} 设置强制缓存 ，但是接口未做后置回调刷新！`);
                }
            })
        }
        ...
        return uni.request({
            ...
            success: (res) => {
                ...
                } else if (res.data.code == config.enumMgr.RESPONSE_CODE.STATION_CLOSE) {
                    //  后端 不允许访问状态码
                    uni.reLaunch({
                        url: "/pages/common/global/errors/ServiceLimitation",
                    });
                    return reject();
                } else if (res.data.code == config.enumMgr.RESPONSE_CODE.ACCESSR_ESTRICTIONS) {
                    //  后端 访问限制状态码
                    uni.reLaunch({
                        url: "/pages/common/accessRestrictions",
                    });
                    return reject();
                } else if (res.data.code == config.enumMgr.RESPONSE_CODE.STATION_STAND) {
                ...
                let afterCacheApiFreshCallBackFn = cacheResult && cacheResult.data ? cacheResult.data[config.enumMgr.API_CACHE_CALLBACK_NAME] : null;
        ...
        return res.request && trytimes > 0 ? resquest(prams, dataObj, headerData, trytimes - 1) : Promise
            .resolve(res)
    });
}
...
function checkResponseVersionToFreshConfig(url, resonse) {
    ...
    let lastConfigsResponse = config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_CONFIG_RESPONSE);
    if (!lastConfigsResponse || resonse.version != lastConfigsResponse.version) {
        uni.removeStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_CONFIG_RESPONSE);
 ...
 * @type {(function(*=))|*}
 */
var oldFunction = uni.uploadFile;
uni.uploadFile = function (prams) {
    ...
    var rewriteOnSuccessCallBack = function (res) {
        ...
        typeof userSetOnSuccessCallBack == "function" && userSetOnSuccessCallBack(res)
    };
