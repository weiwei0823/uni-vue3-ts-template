	...
	<view style="height: 100%;">
		<view :class="showUpdateTime?'zp-r-container zp-r-container-padding':'zp-r-container'">
			<view class="zp-r-left">
				...
				<!-- #ifndef APP-NVUE -->
				<image v-else :class="{'zp-line-loading-image':refresherAnimated}" class="zp-r-left-image" :style="[leftImageStyle,imgStyle]" :src="leftImageSrc" />
				<!-- #endif -->
				<!-- #ifdef APP-NVUE -->
...
<script>
	...
	export default {
		...
		props: ['status', 'defaultThemeStyle', 'defaultText', 'pullingText', 'refreshingText', 'completeText', 'defaultImg', 'pullingImg', 
			'refreshingImg', 'completeImg', 'showUpdateTime', 'updateTimeKey', 'imgStyle', 'titleStyle', 'updateTimeStyle', 'updateTimeTextMap','refresherAnimated'
		],
		computed: {
			ts() {
