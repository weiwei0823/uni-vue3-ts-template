...
const LotteryStore = {
    ...
    getters: {
        ...
        inLotteryHeaderInfoGetTimestamp:state=> state.inLotteryHeaderInfoGetTimestamp,
    ...
    mutations: {
        ...
        SET_LOTTERY_STORE_LOTTERY_GAME_NAME(state,lotteryName){
            ...
            config.setStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_IN_LOTTERY_NAME,lotteryName);
        },
    },
    actions: {
        updateLotteryStoreAfterApiCategoryData({commit,state,dispatch},result){
            let map={};
            result.forEach(item => {
            ...
            if(map[state.inLotteryId]){
               let name = map[state.inLotteryId]['name'];
        ...
        async initLotteryStore({commit}){
           let a = config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_IN_LOTTERY_NAME);
