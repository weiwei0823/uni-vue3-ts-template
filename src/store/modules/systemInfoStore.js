...
const systemInfoStore = {
    ...
    mutations: {
        ...
        SET_WINDOW_BOTTOM: (state, windowBottom) => {
            document.documentElement.style.setProperty('--safe-area-inset-bottom', windowBottom + 'px');
    ...
    actions: {
        ...
        initUniSystemInfo({commit, state, dispatch}) {
            let systemInfo = !state.systemInfo ? uni.getSystemInfoSync() : state.systemInfo;
            let systemHeight = systemInfo.windowHeight;
            dispatch("updateWindowSize", {width: systemInfo.windowWidth, height: systemHeight});
        },
        ...
        setVH({state}) {
            const elH = uni.getStorageSync('downloadShow') === true ? utlilCommons.caculationRpxToPx(108) : 0
