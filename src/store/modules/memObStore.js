...
const _paramKeySort=function (query){
...
const MemObStore = {
    ...
    actions: {
        ...
        updateCacheStore({commit,dispatch,state},{key,info}){
            let {syncTime,cacheTime,data} = info;
            let saveInfo = {syncTime:syncTime||0,cacheTime:cacheTime||0,data:data};
        ...
        doCountApiAndClearCacheAfterClear({commit,dispatch,state},requestArgs){
            let apiName = requestArgs.url;
            if(!apiName){
                console.warn(`${apiName} faild`);
            }
            let oldNum = state.apiCountMap[apiName]||0;
            state.apiCountMap[apiName] = oldNum+1;
            //ApiCache 接口触发后 需要清理其它接口缓存配置 避免数据更新不更新的配置
            let afterClearApis = requestCache.getApiClearCachePointMap();
            let needClearCacehApis = afterClearApis[apiName];
        ...
        bindApiMemCache({commit,dispatch,state},{requestArgs,result,callback}){
            let cachedTime = requestArgs.cacheTime;
            let forceUpdateCache = requestArgs.forceUpdateCache;
            if(!cachedTime && !forceUpdateCache) return;

            let cachePool=state.apiMemCachePool;
            let apiName = requestArgs.url;
            let apiQueryString = _paramKeySort(requestArgs.query);
            ...
            lastCacheDataSlice[API_CACHED_STRINGS.SAVE_DATA_KEY_LIFECRICLE] = Date.now() + parseInt(cachedTime);
        ...
        getApiMemCache({commit,dispatch,state},requestArgs){
            ...
            return new Promise(resolve => {
                ...
                let lastVaildTime = cacheData[API_CACHED_STRINGS.SAVE_DATA_KEY_LIFECRICLE];
                if(timeNow>lastVaildTime && !forceUpdateCache){
                    printDebugLog(`接口缓存失效 ${apiName}`,1,lastVaildTime,timeNow,cachedTime)
                    resolve({status:STATUS.TIMEOUT,data:null});
                    return;
                }
                resolve({status:STATUS.SUCCESS,fromForceUpdateCache:forceUpdateCache,data:cacheData[API_CACHED_STRINGS.SAVE_DATA_KEY_DATA]});
            })
        },
        ...
        clearApiCacheDataAfterCbCallBack({commit,dispatch,state},requestArgs){
            let cachePool=state.apiMemCachePool;
            let apiName = requestArgs.url;
            let apiCache = cachePool[apiName];
            if(apiCache){
                for (var pramsStr in apiCache){
                    let saveData = apiCache[pramsStr];
                    if(!saveData||!saveData.data||!saveData.data[config.enumMgr.API_CACHE_CALLBACK_NAME]) continue;
