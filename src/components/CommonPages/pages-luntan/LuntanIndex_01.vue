<template>
  <view
    class="mint-tab-container-item home-div pc-max-width"
    :style="{
      position: `fixed`,
      height: `${winHeight}px`,
      marginTop: safeHeight,
      width: '100%'
    }"
  ...
  >
    ...
    <scroll-view
      :style="{ backgroundColor: '#fff', height: `calc(var(--vh) - 60rpx)` }"
      :enhanced="true"
      :bounces="false"
    ...
    >
      <view
        class=""
        :style="{
          backgroundColor: themeColor.themeAuxiliaryColorDefault,
          paddingBottom: '20rpx'
        }"
      >
        ...
        <view class="block-lottery">
          ...
          <view class="border-box">
            <view
              ...
              :class="'border-box-body-' + themeColor.name"
              v-if="lastLottery"
            >
              <lottery
                :lotteryDetail="lastLottery"
                :listStyle="{}"
                :listenNumberListAction="true"
                live
                :lotteryType="lotteryType"
                v-on:httpAync="homeAsync"
                isHttpAsync
              >
        ...
        <template
          v-if="
            !$config.configUIMgr.checkPageElementIsHide(
              $config.enumMgr.PAGE_VERSION_ELEMENT_HIDE_KEY
                .EL_LUNTAN_INDEX_02_CATEGORY_01
            )
          "
        >
        ...
        <!--导航栏-->
        <view
          :style="navHeight > 0 ? 'height:' + navHeight + 'px' : ''"
          class="nav-class"
        >
          <view
            id="nav"
            :class="isTop == 1 ? 'tab-div-top tab-div' : 'tab-div'"
        ...
        <view class="forum-body-bd">
          ...
          >
            <ul class="forum-list" v-if="list.length > 0">
              ...
              >
                  ...
                  <view class="user-img" @click="homepage(item)">
                    ...
                    <view class="user-info">
                      <view :class="item.vip == 1 ? 'name red' : 'name'">
                        {{ item.username }}
                        <image
                          v-for="(val, key) in item.medalList"
                          :key="key"
                          :src="val"
                          lazy-load="true"
                        ></image>
                ...
                <view class="forumflow" @click="concern(item)">
                  <span v-if="item.concern == 0">{{
                    $t('components.luntan.index.follow')
                  }}</span>
                  <span v-else>{{
                    $t('components.luntan.index.followed')
                  }}</span>
                </view>
                  ...
                  <view class="van-col van-col--19">
                    <view class="van-ellipsis van19">
                      <span class="up" v-if="item.top == 1">
                        ...
                        <image
                          src="@/static/img/forum/top_n.png"
                          style="width: 68rpx"
                        >
                        </image>
                      ...
                      <span v-if="item.highQuality == 1">
                        <image
                          src="@/static/img/forum/jin_n.png"
                          style="width: 40rpx"
                          class="icon"
                        >
                        </image>
                      ...
                      <view
                        class="formtitle"
                        :style="
                          item.read == 1 ? 'color: rgb(153, 153, 153);' : ''
                        "
                      >
                        {{ item.title }}
                      </view>
                ...
                >
                  <view
                    class="van-ellipsis"
                    v-for="(val, key) in item.descriptionList"
                    :key="key"
                  >
                    {{ val }}
                  </view>
                    ...
                    <view class="van-row img-box-3">
                      ...
                      >
                        <view class="image-body">
                          ...
                          <view
                            v-if="key == 2 && item.pictureList.length > 3"
                            class="length"
                          >
                            {{ $t('components.luntan.index.total')
                            }}{{ item.pictureList.length
                            }}{{ $t('components.luntan.index.cards') }}
                          </view>
                        </view>
                      </view>
                    </view>
                ...
                <view class="tool-ft tom-col van-row">
                  <view class="van-col">
                    ...
                    />
                    <span class="num"
                      >{{ item.thumbUpCount | moneyFormat }}点赞</span
                    >
                  </view>
                  <view class="van-col">
                    <img
                  ...
                  <view class="van-col">
                    ...
                    <span class="num"
                      >{{ item.commentCount | moneyFormat }}评论</span
                    >
                  </view>
                  <view class="ntool van-col van-col--6">
                    <view>
                      ...
                      <img
                        :src="
                          '../../static/img/menu/' + item.lotteryType + '.png'
                        "
                        alt=""
                      />
                      <span class="num" style="color: #404040">{{
                        lotteryName(item.lotteryType)
                      }}</span>
                    </view>
                  </view>
                </view>
              ...
              >
                <view
                  class=""
                  style="height: 2rpx; width: 60rpx; background-color: #a1a1a1"
                />
                <view
                  class=""
                  style="font-size: 28rpx; color: #a1a1a1; margin: 0 20rpx"
                >
                  没有更多了
                </view>
                <view
            ...
            <empty style="height: 800rpx" v-else> </empty>
    ...
    >
      <LotteryTypeTab
        :showTypeImage="lotteryTime.length <= 4"
...
export default {
  ...
  data() {
    return {
      ...
      sortList: [
        this.$t('components.luntan.index.comprehensive'),
        this.$t('components.luntan.index.essence'),
        this.$t('components.luntan.index.mostLikes'),
        this.$t('components.luntan.index.newest')
      ],
      navList: [
        {
          lotteryType: 0,
  ...
  computed: {
    ...
    sortLotteryTypeTabs() {
      const buildAr = []
      for (const key in this.lotteryTime) {
        const lotteryInfo = this.lotteryTime[key]
        const copyLotteryInfo = { ...lotteryInfo }
        copyLotteryInfo.sortIndex =
          this.$config.lotterTimesTabsSort &&
          Array.isArray(this.$config.lotterTimesTabsSort)
            ? this.$config.lotterTimesTabsSort.indexOf(
                Number(copyLotteryInfo.lotteryType)
              )
            : 0
  ...
  methods: {
    homeAsync() {
      ...
      lastLotteryRecordWithCalculate(prams).then((res) => {
        ...
        this.$store.commit('SET_OPEN_LOTTERY', { ...this.lastLottery })
      })
    },
    _onLoad() {
      ...
      uni.$on('preloadLuntanHomeData', (res) => {
        this.$requestCache.bindLastCacheUpdateApiCallBack(
          res,
          (netResp) => {
      ...
      this.lotterytype = this.$config.getStorageSync(
        ...
        this.$config.defaultValues.DEFAULT_SELECT_LOTTERY_TYPE
      )
    ...
    freshUIByConfigData() {
      const configData = this.$config.getStorageSync(
      ...
      lt_init(prams)
        .then((res) => {
          _this.$requestCache.bindLastCacheUpdateApiCallBack(
            res,
            (netResp) => {
              _this.doUpdateUiAfterInited(noRefrsh, netResp)
            },
            this
          )
    ...
    doUpdateUiAfterInited(noRefrsh, res) {
      ...
      this.noticeList = data.noticeList.map((item) =>
        item.noticeContent.replace(/\n/g, '')
      )
      this.listPicture = data.advertList
      this.lotteryTime = data.lotteryTimeList
      if (this.lotteryTime.length >= 1) {
        const memLotteryId = this.$config.getStorageSync(
          this.$config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_LOTTERYTYPE,
          2
        )
        const inLottery = this.lotteryTime.find((item) => {
          return item.lotteryType == memLotteryId
        })
        if (!inLottery) {
          this.$config.setStorageSync(
            this.$config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_LOTTERYTYPE,
            this.lotteryTime[0].lotteryType
          )
          this.lotterytype = this.$config.getStorageSync(
            this.$config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_LOTTERYTYPE,
            this.lotteryTime[0].lotteryType
      ...
      if (!noRefrsh) {
        const listLenth = data.listPicture.length
        const leftArr = []
        const rightArr = []
    ...
    detail(item) {
      ...
      item.read = 1
      uni.navigateTo({
        url: `/pages/luntan/masterForum/detail?id=${item.articleId}&type=${item.type}&createTime=${item.createTime}`,
        success: (res) => {
    ...
    // 关注接口
    concern(item) {
      const token = this.$config.getStorageSync(
    ...
    // 年跳转
    picture(item) {
      const url = `/pages/luntan/liuhePicture/index?year=${item.year}&color=${item.color}`
      uni.navigateTo({
        url
      })
    },
    // 图库详情
    pictureDetail(item) {
      const url = `/pages/luntan/liuhePicture/detail?id=${item.pictureId}`
      uni.navigateTo({
        url
      })
    },
    ...
    // 动态设置swiper的高度
    setSwiperHeight() {
      const element = `#content-wrap${this.current}`
...
.border-box-body-blue {
  background: linear-gradient(
    180deg,
    var(--theme-auxiliary-color-linear-gradient-start) 0%,
    var(--theme-auxiliary-color-linear-gradient-end) 100%
  );
}
