<template>
	<view>
		<view mode="aspectFill" :src="icon_path" class="fixedView icon" @touchstart.stop="touchIcon"
			@touchmove.stop.prevent="moveIcon" @tap.stop="tapIcon"
			...
			<svg style="width: 100%; height: 100%;" xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44" fill="none">
...
export default {
	...
	mounted() {
		...
		switch (this.startPostion) {
			...
			default:
				//初始位置右下角
				const maxLeft = uni.getSystemInfoSync().windowWidth / 2 + this.iconSize.width
				this.$set(this.iconPosition, 'left', maxLeft)
				this.$set(this.iconPosition, 'top', this.windowSize.height - uni.upx2px(this.iconSize.height) - this.windowSize.margin.bottom);
				break;
		}
		var that = this;
	...
	methods: {
		...
		moveIcon(e) {
			...
			const minLeft = uni.getSystemInfoSync().windowWidth / 2 - this.iconSize.width * 1.5
			const maxLeft = uni.getSystemInfoSync().windowWidth / 2 + this.iconSize.width
			if (this.iconPosition.left <= minLeft) {
				this.$set(this.iconPosition, 'left', minLeft)
			}
			if (this.iconPosition.left >= maxLeft) {
				this.$set(this.iconPosition, 'left', maxLeft)
			}
...
.icon {
	...
	overflow: hidden;
	border: none;
	// box-shadow: 0px 2px 5px #A1A1A1 !important;
}
</style>
