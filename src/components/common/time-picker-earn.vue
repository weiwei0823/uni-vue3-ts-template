<template>
	<view class="date-picker" :class="['theme-' + themeColor.name]" :style="{ height: uWindowHeight }">
		<Header :title="title" :isBack="false" showBack @back="back">
			<block slot="right">
				<view class="confirm" @click="cofim">{{ $t('pages.common.timePicker.confirm') }}</view>
			</block>
		</Header>
		<view class="content" :style="contentStyle">
			...
			<view class="calendar">
				<view class="header">
					<svg @click="updateSelectedDate('up')" xmlns="http://www.w3.org/2000/svg" width="22" height="22"
					...
					<view class="title">{{ $u.timeFormat(selected_date, 'yyyy年mm月') }}</view>
					<svg @click="updateSelectedDate('next')" xmlns="http://www.w3.org/2000/svg" width="22" height="22"
						viewBox="0 0 22 22" fill="none">
				...
				<view class="days">
					<view class="day" :class="{
						...
						<view v-if="day" class="text" :class="{ selected: isSelected(day), locked: caculationIsLocked(day) }"
...
export default {
	...
	methods: {
		...
		isBgFirst(day) {
			return new Date(day).getTime() === new Date(this.start_date).getTime() || new Date(day).getDay() === 0
		},
		isBgLast(day) {
			return new Date(day).getTime() === new Date(this.end_date).getTime() || new Date(day).getDay() === 6
		},
		isGrayRounded(day) {
		...
		// 是否选中
		isSelected(day) {
			return day.getTime() === this.start_date?.getTime() || day.getTime() === this.end_date?.getTime()
		},
		caculationIsLocked(day) {
			if (this.rangeTodayBeforeDays <= 0) return false;
			else {
				let nowTime = new Date().getTime();
				let timestampItem = new Date(day).getTime();
				if (nowTime < timestampItem || (nowTime - timestampItem >= (24 * 3600 * 1000 * this
					.rangeTodayBeforeDays))) {
					return true;
				}
			}
			...
			if (this.selectDayRange <= 0) return false;
				...
				let timestampStartDate = new Date(this.start_date).getTime();
				let timestampItem = new Date(day).getTime();
				if (timestampItem - timestampStartDate >= (24 * 3600 * 1000 * this.selectDayRange)) {
					return true;
				}
			}
		...
		handleSelect(day) {
			...
			if (!this.start_date && !this.end_date) {
				...
				// 如果超过30天
				if (day.getTime() - this.start_date.getTime() > 30 * 24 * 60 * 60 * 1000) {
					pop('不能选择超过30天的日期')
					return
				}
				this.end_date = day
