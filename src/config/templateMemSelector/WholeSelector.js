...
export default {
    ...
    init(){
        ...
        try {
            ...
            request.onupgradeneeded = function(event) {
                ...
                var objectStore = db.createObjectStore('lotteryPlays', { keyPath: 'id' });
    ...
    saveLotteryMiniPlay(id,playList){
        ...
        let request= this.db.transaction(["lotteryPlays"],'readwrite').objectStore('lotteryPlays').put({
            id: Number(id),
    ...
    readLotteiesPlayFromDb(){
        ...
        let _this = this;
        let configId = config.configLotties.lotteryTypeIncludeIdsMap;
        for (let lotteryType in configId){
            let ids = configId[lotteryType];
            ids.map(id=>{
                let r=  _this.db.transaction(["lotteryPlays"],'readwrite').objectStore('lotteryPlays').get(Number(id));
                r.onsuccess=function (event){
                    if(event.target.result){
                        try {
                            let memMap = JSON.parse(event.target.result.playList);
    ...
    getMemeLotteryByLotteryId(lotteryId) {
        let _this = this;
        let contantsConfigMaps = $store.state.appInfoStore.contantsConfigTypeMap;
        let url = contantsConfigMaps[config.enumMgr.CONTANTS_CONFIGS_ID.IMAGE_URL];
        let projectVersion = contantsConfigMaps[config.enumMgr.CONTANTS_CONFIGS_ID.PROJECT_VERSION];
        return new Promise((resolve) => {
            if (!url) {
                console.error(` 常量配置中[${config.enumMgr.CONTANTS_CONFIGS_ID.IMAGE_URL}]缺失`);
                resolve(null);
            } else if (!projectVersion) {
                console.error(` 常量配置中[${config.enumMgr.CONTANTS_CONFIGS_ID.PROJECT_VERSION}]缺失`);
                resolve(null);
            } else {
                let wholeUrl = `${url.url}${projectVersion.url.toLowerCase()}/json/play_${lotteryId}.json`
                ...
                uni.request({
                        ...
                        success: (res) => {
                            if (res.statusCode == 200) {
                                ...
                                let resultCode = decryptResult(result);
