...
export default {
    ...
    getJumpRouteByType(lotteryType, lotteryId, isLive) {
        let isLiveFlag = (!!isLive && isLive != 'false') ? 1 : 0;
        ...
        let url = `${gameInfo.route}?id=${lotteryId}&isLive=${isLiveFlag}`
        return url;
    },
    /**
     * 获取当前所有游戏的path
    ...
    getAllGamePaths() {
        let ar = [];
        for (let k in this.routeConfigs) {
    ...
    doJumpLotteryGameAndUpdateGameTitle(gameInfo, emitArgEvent = false) {
        ...
        let url = this.getJumpRouteByType(gameInfo.lotteryTypeId, gameInfo.lotteryId, gameInfo.isLive);
        /**
         * @MR 20240113 修改
        ...
        uni.redirectTo({
            ...
            success: function (res) {
                emitArgEvent && res.eventChannel && res.eventChannel.emit('argParams', {
     ...
     */
    doOpenGameDetails(gameDateItem, pageInstance, filterRecharge = true, filterLogin = true) {
        const { openGameIntercept, userInfo } = pageInstance.$store.state.appInfoStore
        const jumpType = (gameDateItem.jumpType || gameDateItem.type) ?? 2
        /**
         * 如果为1类型自带彩票 调用此方式打开
         */
        if (jumpType == 1) {
            let useGameData = gameDateItem['lotteryId'] ? gameDateItem : gameDateItem['data'];
            ...
            let configInfo = $store.state.appInfoStore.contantsConfigTypeMap[config.enumMgr.CONTANTS_CONFIGS_ID.ENTER_GAME_CONDITION];
            ...
            const gamePlatformId = gameDateItem.platformId || gameDateItem.gamePlatformId;
            const platformCateId = gameDateItem.platformCateId || gameDateItem.gamePlatformCateId;
            this.doEnterWebviewGame(pageInstance, code, gamePlatformId, platformCateId, gameDateItem.name, gameDateItem)
        } else {
            if (gameDateItem.customJumpType == 1) {
                uni.switchTab({
    ...
    doEnterWebviewGame(pageInstance, gameCode, platformId, platformCateId, gameName, gameDateItem) {
        /**
         * 在锁定时间内不允许重复点击
         */
        ...
        enter(prams).then(res => {
            if (res.code == 200) {
                ...
                // }
                const h5url = '/pages/liuhe/webview/game?platformId=' + platformId + '&url=' + encodeURIComponent(res.result) + '&backPath=' + pageInstance.$route.path + '&gameName=' + gameName;
                // AG BG游戏会出现加载成pc样式，导致在安卓上会出现异常样式，所以这改为原生跳转
                if (browserPlatform() == browserPlatform.FLUTTER_App && [5, 6, 10].includes(gameDateItem.platformId)) {
                    ...
                    window.flutter_inappwebview?.callHandler("openInnerExtraUrl", u, gameName)
