...
export default {
	...
	freshLotteryTypeIncludeIdsMapAfterConfig(lotteryTypeIncludeIdsMap){
		if(lotteryTypeIncludeIdsMap){
			for (let lotteryTypeId in lotteryTypeIncludeIdsMap){
				let lotteryIds=lotteryTypeIncludeIdsMap[lotteryTypeId];
				if(lotteryIds&&lotteryIds instanceof Array){
					lotteryIds.forEach((id)=>{
						if(this.lotteryTypeIncludeIdsMap[lotteryTypeId]&&this.lotteryTypeIncludeIdsMap[lotteryTypeId].indexOf(id)==-1){
	...
	getLotteryTypeTimeNameByLotteryTypeId(typeId){
		...
		switch (typeId) {
			case 1:
				return app.$t('pages.common.userCenter.index.collection.liuheGalleryHongKong')
			case 2:
				return app.$t('pages.common.userCenter.index.collection.liuheGalleryMacao')
			case 3:
				return app.$t('pages.common.userCenter.index.collection.liuheGalleryTaiwan')
			case 4:
				return app.$t('pages.common.userCenter.index.collection.liuheGallerySingapore')
			case 5:
