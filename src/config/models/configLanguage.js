...
export default {
	...
	loadLocaleMessages() {
		const locales = require.context('../../locale', true, /[A-Za-z0-9-_,\s]+\/import\.js$/i)
		const messages = {}
		locales.keys().forEach(key => {
			messages[key.replace('./', '').replace('/import.js', '')] = locales(key).default
		})
		return messages
	},
	setLocalLanguage(userDefaultLanguageValue) {
		let defaultKey = uni.getStorageSync(enumConfig.APP_LOCALSTORE_KEYS.LANGUAGE_SET_VALUE);
		if (!defaultKey) {
			defaultKey = userDefaultLanguageValue
			// uni.setStorageSync(enumConfig.APP_LOCALSTORE_KEYS.LANGUAGE_SET_VALUE, defaultKey);
		}
		const strLang = this.languageSelect.find(item => item.id == defaultKey).value;
	...
	getEnvLocal(configWebsite) {
		...
		switch (config.station) {
			case config.enumMgr.STATION.ALL:
				if (configWebsite.all_luntan_language_auto === 1) return this.formatterEnvLocal()
				break;
			case config.enumMgr.STATION.LUNTAN:
				if (configWebsite.lluntan_language_auto === 1) return this.formatterEnvLocal()
				break;
			case config.enumMgr.STATION.LIUHE:
				if (configWebsite.liuhe_language_auto === 1) return this.formatterEnvLocal()
				break;
		}
	},
	formatterEnvLocal() {
		...
		const lang = _this.languageSelect.find(item => item.navigatorStr == res.language);
		return lang?.id || 2;

	},
	setDefalutLocalLanguage() {
		const configWebsite = config.getStorageSync(config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_WEBSITECONFIG);
		let defaultKey = null;
		// 新逻辑，先判断环境
		...
		if (!config.isForceServerLanguage && !defaultKey) {
			const setKey = uni.getStorageSync(enumConfig.APP_LOCALSTORE_KEYS.LANGUAGE_SET_VALUE);
			if (setKey) {
		...
		let languageConfig = this.languageSelect.find(item => item.id === defaultKey);
		if(!languageConfig){
			console.error(`respose language_settting= ${defaultKey} but language list undefined this id`);
