...
const isDebug = configuratioBuilder.getConfigIsDebugOrDefault(false) // true;
// 是否为开发环境
const isDev = configuratioBuilder.getConfigIsDevOrDefault(true)
// 是否强制使用服务端主题
const isForceServerTheme = configuratioBuilder.getConfigIsForceServerTheme(true)
// 是否强制使用服务端语言
const isForceServerLanguage =
  configuratioBuilder.getConfigIsForceServerLanguage(true)
// 当前站点对应环境

let station = configEnums.STATION.LUNTAN // ALL LIUHE LUNTAN
...
// 是否开启HTTP 发送消息加密
const allHttpRequestUseEncrypt =
  configuratioBuilder.getConfigUseEncyptOnHttpReq(true)
// 是否开启HTTP 打印发送消息加密日志
const allHttpRequestUseEncryptPrintLog =
  configuratioBuilder.getConfigUseEncyptOnHttpReqPrintLog(!!isDebug)
// 是否开启HTTP 返回消息需要解密
const allHttpResponseUserEncrypt =
  configuratioBuilder.getConfigUseEncyptOnHttpResp(true)

// 是否打开HTTP本地缓存
const allHttpRequestUseCacheMap =
  configuratioBuilder.getConfigUseHttpCacheMap(true)

// 是否开启Sign加密流程
const stationOpenHttpSign = configuratioBuilder.getConfigOpenHttpSign(!!isDebug)
 ...
 * @type {string}
 */
const apiDomainPool = isDebug ? [''] : ['']
const apiDomain = apiDomainPool[0]

// 设置页面显示版本号 生产环境鉴别版本使用
const versionPro = 'v2.31.36'
// 个人中心页面显示版本号 测试环境显示生产不显示 用于测试高频发布鉴别版本使用

const versionDev = 'v2.31.36_2024062501'

/**
 * websocket 连接池配置 方便后期扩张检测链接最快速度
 */
const websocketWsUrls = isDebug
  ? isDev
    ? ['https://zh.56888.bet', 'http://devh5.558888666.com']
    : ['https://66136.cc']
  : ['']
let websocketWsUrl = websocketWsUrls[0]
// #ifdef H5
if (websocketWsUrl == '') {
...
const config = {
  ...
  allHttpRequestUseEncrypt,
  // 是否所有打印所有request加密明文日志
  allHttpRequestUseEncryptPrintLog,
  // 是否开启HTTP 返回消息需要解密
  allHttpResponseUserEncrypt,
  // 是否开始HTTP 接口缓存匹配
  allHttpRequestUseCacheMap,
  ...
  updateDomain(apiDomain = null) {
    ...
    // 将url写入
    const url =
      apiDomain && apiDomain.url
        ? apiDomain.url[apiDomain.url.length - 1] == '/'
          ? apiDomain.url.slice(0, -1)
          : apiDomain.url
        : null
  ...
  compriseLiuheStation() {
    return (
      this.station == configEnums.STATION.ALL ||
      this.station == configEnums.STATION.LIUHE
    )
  },
  ...
  queryParamFromSeach(parameterName) {
    // #ifdef H5
    const search =
      window && window.location.search ? window.location.search.slice(1) : null
    if (!search) return null
    let queryMap = this.queryParamFromSeachTmp[search]
    if (!queryMap) {
      queryMap = {}
      const querysStrings = search.split('&')
      for (const k in querysStrings) {
        const queryString = querysStrings[k]
        if (queryString.indexOf('=') == -1) continue
        const querySplited = queryString.split('=')
        if (!querySplited || querySplited.length != 2) {
          console.warn('split queryString error')
          continue
        }
        const paramename = querySplited[0]
        const paramevalue = querySplited[1]
  ...
  getStorageSync(key, defaultValue, initAfterDefault = false) {
    ...
    this.showLocalStorageReadLog &&
      console.log(
        `【localstorage:read】  key = ${key} value=`,
        value,
  ...
  setStorageSync(key, value, tryClearIfWriteError = false) {
    this.showLocalStorageReadLog &&
      console.log(`【localstorage:write】  key = ${key} value=`, value)
    uni.setStorage({
      key,
      data: value,
      fail: () => {
        if (tryClearIfWriteError) {
          ...
          uni.removeStorageSync(
            config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_DEBUG_LOG_HTTP
          )
  ...
  clearStorage(isClearUser, reload = true) {
    ...
    if (!isClearUser) {
      const mem = {}
      for (const key in noClearUserMessage) {
        const storageKey = noClearUserMessage[key]
  ...
  autoClearStorageBeforeOpen() {
    const version = this.getStorageSync(
      this.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_VERSION_KEY
    )
  ...
  initBuildSignModule() {
    if (
      this.stationOpenHttpSign &&
      window.initSignModel &&
      window.initSignModel instanceof Function
    ) {
      initSignModel()
    }
  }
