...
//是否强制使用服务端语言
var isForceServerLanguage =
  configuratioBuilder.getConfigIsForceServerLanguage(true)
// 当前站点对应环境
let station = configEnums.STATION.ALL //ALL LIUHE LUNTAN
...
var stationVersionCode =
  configuratioBuilder.getConfigStationVersionCodeOrDefault(
    configEnums.STATION_VERSION_CODE.DEFAULT_CODE
  )
// 是否开启HTTP 发送消息加密
let allHttpRequestUseEncrypt =
  configuratioBuilder.getConfigUseEncyptOnHttpReq(true)
// 是否开启HTTP 打印发送消息加密日志
let allHttpRequestUseEncryptPrintLog =
  configuratioBuilder.getConfigUseEncyptOnHttpReqPrintLog(
    isDebug ? true : false
  )
// 是否开启HTTP 返回消息需要解密
let allHttpResponseUserEncrypt =
  configuratioBuilder.getConfigUseEncyptOnHttpResp(true)

// 是否打开HTTP本地缓存
let allHttpRequestUseCacheMap =
  configuratioBuilder.getConfigUseHttpCacheMap(true)

// 是否开启Sign加密流程
let stationOpenHttpSign = configuratioBuilder.getConfigOpenHttpSign(!!isDebug);
)
...
let versionDev = 'v2.31.1_2024071001';
 ...
 */
let websocketWsUrls = isDebug
  ? isDev
    ? ['https://h5.test-mk.net', 'https://h5.test-mk.net']
...
const config = {
  ...
  autoClearStorageBeforeOpen() {
    let version = this.getStorageSync(
