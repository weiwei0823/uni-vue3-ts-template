<template>
	<view class="log-box" :style="{ marginTop: safeHeight }" :class="['theme-' + themeColor.name]">
		...
		<view class="type-box">
			<view class="row-box" :style="{ borderRadius: isShow || list.length ? '' : '0 0 24rpx 24rpx' }">
				<view class="title icon-box" @click="isShow = !isShow">
					<view class="text">{{ typeTitle }}</view>
					<svg :style="{
						transition: 'all .3s',
				...
				<view class="title icon-box">
					{{ $t('pages.common.userCenter.accountLog.giftMoney.EffectiveStatistics') }}
				</view>
				<view class="title icon-box" @click="setSort('amount')">
					{{ $t('pages.common.userCenter.accountLog.giftMoney.giftMoney') }}
					<view class="sort-box">
				...
				<view class="title icon-box" @click="setSort('create_time')">
					{{ $t('pages.common.userCenter.accountLog.giftMoney.time') }}
					<view class="sort-box">
						<svg width="19" height="18" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect x="0.5" width="18" height="18" rx="9" :fill="themeColor.themeAuxiliaryColorDefault" />
							<path
			...
			<view class="container-data" v-show="isShow">
				<view class="container-box" :class="isShow ? 'fadeOutDown' : 'fadeOutUp'">
					<view class="container-row">
						<view class="container-list">
							<view class="container-list-item"
								:class="{ active: typeTitle == $t('pages.common.userCenter.accountLog.index.All') }"
								@click="setGroupType()">
								{{ $t('pages.common.userCenter.accountLog.index.All') }}
							</view>
							<view class="container-list-item" v-for="(item, index) in GroupType"
								:class="{ active: typeTitle == item.title }" @click="setGroupType(item)">
								{{ item.title }}
							</view>
						</view>
					</view>
				...
				<view class="list-item-box">
					<view class="list-item" v-for="(item, index) in list" @click="detail(item)">
						...
						<view class="new" v-if="item.timeText == '刚刚'">
							<svg width="27" height="14" viewBox="0 0 27 14" fill="none"
								...
								<path
									...
									fill="#A1A1A1" />
								<defs>
									<linearGradient id="paint0_linear_15862_194359" x1="0" y1="13.6709" x2="28"
										y2="13.6709" gradientUnits="userSpaceOnUse">
						...
						<uni-row class="row-box list-row">
							...
							<uni-col class="row-col" :span="6">
								<view class="title" style="justify-content: center;">
									<!--           要求有效统计都显示0       <view class="bank" :class="item.amount >= 0 ? 'red' : 'green'">{{ item.amount }}</view>-->
									<view class="bank">0.00</view>
								</view>
							</uni-col>
							<uni-col class="row-col" :span="6">
								<view class="title" style="justify-content: center;">
									<view class="bank" :class="item.amount >= 0 ? 'red' : 'green'">{{ item.amount }}
									</view>
								</view>
							</uni-col>
							<uni-col class="row-col" :span="6">
								<view class="title time-box" style="align-items: flex-end;">
		...
		methods: {
			...
			setGroupType(item = null) {
				if (item != null) {
					...
					this.code = ""
					this.typeTitle = this.$t('pages.common.userCenter.accountLog.index.All')
				}
				this.isShow = false
			...
			getList(pageNo, pageSize = 10) {
				...
				// !this.firstLoadingFinished && (this.loading = true);
