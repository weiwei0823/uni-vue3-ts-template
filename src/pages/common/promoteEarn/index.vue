<template>
	<view class="ptp" :style="{ height: `${uWindowHeight}px` }" :class="['theme-' + themeColor.name]">
		<Header :title="title" isFixed :useCustomBack="true" @back="back" style="z-index:100;" />
		<view class="content">
			<view class="header">
				<view class="left">
			...
			<view class="flex">
				<view class="nav">
					<view class="item" :class="{ active: navActive === item.key }" v-for="(item) in navList"
						:key="item.key" @click="navActive = item.key">
						{{ item.name }}
					</view>
					<view class="item btns" :style="{ backgroundColor: item.color }" v-for="(item) in btns"
						:key="item.key" @click="handleBtnClick(item)">
						{{ item.name }}
					</view>
				</view>
				<scroll-view
					:style="{ height: isApp ? `calc(${uWindowHeight}px - 300rpx)` : 'calc(var(--vh) - 270rpx)' }"
				...
				<svg v-if="scrollTop > 100" class="back-top-icon" @click="scrollTop = 0"
