<template>
	<view class="pc-max-width"
		...
		<scroll-view scroll-y class="scroll-y" :style="{height:`calc(${winHeight}px - 280rpx - var(--safe-area-inset-bottom))`}" :enhanced="true"
			...
			<Empty v-else-if="!tableData.rols" style="height: 100%"></Empty>
			<view class="table-content" v-else>
				<table>
					...
					<tr v-for="(item, index) in tableData.rols" :key="index">
						...
						<td :class="item.normal1 == $t('pages.common.informationCount.jqys.text0') || 'red'">
							{{ item.normal1 }}
						</td>
						<td :class="item.normal2 == $t('pages.common.informationCount.jqys.text0') || 'red'">
							{{ item.normal2 }}
						</td>
						<td :class="item.normal3 == $t('pages.common.informationCount.jqys.text0') || 'red'">
							{{ item.normal3 }}
						</td>
						<td :class="item.normal4 == $t('pages.common.informationCount.jqys.text0') || 'red'">
							{{ item.normal4 }}
						</td>
						<td :class="item.normal5 == $t('pages.common.informationCount.jqys.text0') || 'red'">
							{{ item.normal5 }}
						</td>
						<td :class="item.normal6 == $t('pages.common.informationCount.jqys.text0') || 'red'">
							{{ item.normal6 }}
						</td>
						<td :class="item.special == $t('pages.common.informationCount.jqys.text0') || 'red'">
							{{ item.special }}
						</td>
					</tr>
				</table>
...
<script>
	...
	export default {
		...
		data() {
			...
			return {
				...
				safeHeight: this.$config.getStorageSync(this.$config.enumMgr.APP_LOCALSTORE_KEYS.SYSTEM_INFO).safeArea
					.top + "px",
				title: this.$t('pages.common.informationCount.jqys.title'),
				lotteryType: 1,
				years: '',
				index: 0,
				array: [],
		...
		methods: {
			...
			animalAnalyze: function() {
				this.loading = true
				let prams = {
					year: this.years
				};
				animalAnalyze(prams).then(res => {
					this.loading = false
