		...
		<Header :title="title" :isFixed="true">
			<block slot="right">
				<view class="uni-list-cell-db" style="display:flex;justify-content: flex-end;">
					<text class="uni-subtitle">{{ $t('pages.common.informationCount.tmls.text0') }}</text>
					<input class="uni-input" v-model="period" @blur="numberStat" @confirm="numberStat" />
				</view>
			</block>
		</Header>
		<scroll-view scroll-y class="scroll-y"
			:style="{height:`calc(${winHeight}px - 88rpx - 145rpx - var(--safe-area-inset-bottom))`}" :enhanced="true"
			:bounces="false" :show-scrollbar="false">
			<!--列表-->
			<LuntanLoading class="loading" v-if="loading" />
			<view class="mint-tab-container-item">
				<view class="title-box">
					<h4>{{ $t('pages.common.informationCount.tmls.text1') }}：{{ period }}</h4>
					<p>{{ $t('pages.common.informationCount.tmls.text2') }}</p>
				</view>
				<view id="chart" class="charts-box">
					<l-echart ref="chart"></l-echart>
				</view>
				<view class="title-box">
					<p style="margin-bottom: 10rpx;">{{ $t('pages.common.informationCount.tmls.text3') }}</p>
				</view>
		...
		data() {
			return {
				...
				safeHeight: this.$config.getStorageSync(this.$config.enumMgr.APP_LOCALSTORE_KEYS.SYSTEM_INFO).safeArea
					.top + "px",
				lotteryType: 1,
				title: this.$t('pages.common.informationCount.tmls.title'),
				type: 1,
				period: 100,
				chartDataHot: {
					backgroundColor: "#fff",
					...
					//直角坐标系 grid 中的 x 轴，
					xAxis: {
						name: this.$t('pages.common.informationCount.tmls.text4'),
					...
					//直角坐标系 grid 中的 y 轴，
					yAxis: {
						name: this.$t('pages.common.informationCount.tmls.text5'),
				...
				chartDataCold: {
					...
					//直角坐标系 grid 中的 x 轴，
					xAxis: {
						name: this.$t('pages.common.informationCount.tmls.text6'),
						nameLocation: 'start', //有start  和end 分别为左右
						nameTextStyle: {
							align: "center",
							verticalAlign: "top",
					...
					//直角坐标系 grid 中的 y 轴，
					yAxis: {
						name: this.$t('pages.common.informationCount.tmls.text7'),
						type: 'value', //'value' 数值轴，适用于连续数据。
						nameTextStyle: {
							align: "right"
						},
