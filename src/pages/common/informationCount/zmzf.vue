	...
	<view class="mint-tab-container-item" :style="{ marginTop: safeHeight }" :class="['theme-' + themeColor.name]">
		...
		<Header :title="title">
			<block slot="right">
				<view class="uni-list-cell-db" style="justify-content: flex-end;">
					<text class="uni-subtitle">{{ $t('pages.common.informationCount.zmzf.text0') }}</text>
		...
		<view class="mint-tab-container-item block-content">
			<view class="title-box">
				<h4>{{ $t('pages.common.informationCount.zmzf.text1') }}：{{ period }}</h4>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				safeHeight: this.$config.getStorageSync(this.$config.enumMgr.APP_LOCALSTORE_KEYS.SYSTEM_INFO).safeArea
					.top + "px",
				lotteryType: 1,
				title: this.$t('pages.common.informationCount.zmzf.title'),
				type: 1,
				period: 100,
				chartDataHot: {
					backgroundColor: "#fff",
					...
					//直角坐标系 grid 中的 x 轴，
					xAxis: {
						name: this.$t('pages.common.informationCount.zmzf.text2'),
					...
					//直角坐标系 grid 中的 y 轴，
					yAxis: {
						name: this.$t('pages.common.informationCount.zmzf.text3'),
		...
		methods: {
			getDeviceEchart() {
				...
				// optionHot.series[0].itemStyle = this.themeColor.themeColor
