<template>
	<view class="mint-tab-container-item pc-max-width"
		...
		<!--顶部导航栏-->
		<Header :title="title" isFixed :headerStyle="{borderRadius:0,borderBottom:0}">
			<block slot="right">
		...
		<scroll-view scroll-y class="scroll-y"
			...
			<view class="mint-tab-container-item " style="background: #f4f4f4;">
				<view class="tab-wrap">
		...
		data() {
			return {
				...
				safeHeight: this.$config.getStorageSync(this.$config.enumMgr.APP_LOCALSTORE_KEYS.SYSTEM_INFO).safeArea
					.top + "px",
				lotteryType: 1,
				title: this.$t('pages.common.informationCount.tmws.title'),
				period: 100,
				isInt: false,
				type: 2,
				isActive: 0,
