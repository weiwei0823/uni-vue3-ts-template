...
<script>
	...
	export default {
		...
		computed: {
			...
			selectedIds() {
				let orderIdListInOrderList = this.orderList.length ? this.orderList.map(i => i.playId) : [];
				let orderIdListInOrderContent = [];
				this.orderList.length ? this.orderList.forEach((i) => {
					let contentSplit = i.content.split(',');
					contentSplit.forEach((contentItem) => {
						if (!isNaN(parseInt(contentItem))) orderIdListInOrderContent.push(parseInt(
							contentItem));
					})
				}) : [];
				return orderIdListInOrderList.length > orderIdListInOrderContent.length ? orderIdListInOrderList :
					orderIdListInOrderContent;
			}
		},
		components: {
			UniCol,
		...
		methods: {
			setDefaultSelected() {
				...
				if (this.orderList.length) {
					if (this.orderList[0].content === "") {
						selectedIds = this.orderList.length ? this.orderList.map(i => i.playId) : []
					} else {
						selectedIds = this.orderList.length ? this.orderList[0].content.split(',').map(i => Number(i)) : []
					}
				}
			...
			onClickItem(item) {
				this.currentArr = this.playing.setSelectedArr(this.currentArr, item); //去重后的选择id
