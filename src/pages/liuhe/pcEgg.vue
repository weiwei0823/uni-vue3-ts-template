				...
				<view class="board" :style="{paddingTop:`${keyBoardHeight}px`}">
					...
					<view class="board-content">
						<view class="board-content-item-left">
							<view class="top">
								<text v-if="showRoundInfo.stampType === 3">{{ `${$t("pages.liuhe.common.text1")}${showRoundInfo.nextShorNo ? showRoundInfo.nextShorNo : ''}${$t("pages.liuhe.liuhePlaying.text2")}`
                  }}</text>
								<text
							...
							<view class="ball-list">
								<view class="ball-list-item"
									...
									:key="index">
									<view class="ball-num">
										<text class="text">{{ headInfo.stampType === 2 ? item : "?" }}</text>
									</view>
									<text class="ball-add" v-if="index !==2">+</text>
									<text class="ball-add" v-else>=</text>
								</view>
								<view class="ball-num-result"
									:class="ballColor === '1' ? 'ball-red': ( ballColor === '2' ? 'ball-blue' : ( ballColor === '3' ? 'ball-green' : 'ball-gray' ) )">
									<text class="text" v-if="headInfo.stampType === 2">{{ totalNumber }}</text>
									<view class="ball-text" v-else>
										<image :src="require('./../../static/img/liuhe/questionMark.png')"></image>
									</view>
								</view>
							</view>
							<view class="result-detail" v-if="headInfo.stampType === 2">
								{{ headInfo.resultDetail && headInfo.resultDetail.split('_')[1] }}
							</view>
						</view>
...
<script>
	...
	import BettingArea from "./pcEgg/betting";
	import Buy from "./common/buy";
	import Records from "./pcEgg/records";
	...
	export default {
		...
		data() {
			return {
				...
				navBg: require('@/static/img/liuhe/bg-nav.svg'),
      ...
      ...mapState(useWebsocketStore, ["websocketTimeInterval"]),
			...
			totalNumber() {
				return this.headInfo.lotteryResult && this.headInfo.resultDetail.split('_')[0].split(' ')[0] || ""
			},
			ballColor() {
      ...
      if (isLogin()) {
		...
		methods: {
			...
			setTabStyle() {
				return {
					height: this.uWindowHeight - this.systemInfo.statusBarHeight - 100 + 'px'
				}
			},
			setAmount(num) {
			...
			toUrl(item) {
				...
				if (item.code === "recent") {
					item.url = "/pages/liuhe/pcEgg/recent_draws"
					uni.navigateTo({
