<template>
	<view class="ball-list">
		<view :style="setItemStyle(index, ballChildren.length)" v-for="(item, index) in ballChildren"
	...
	export default {
		...
		methods: {
			setDefaultSelected() {
				...
				let selectedIdsContent = this.orderList.length && this.orderList[0].content ? this.orderList[0].content
					.split(',').map(i => {
						return Number(i)
					}) : [];

				let selectedIds = selectedIdsOrder.length >= selectedIdsContent.length ? selectedIdsOrder :
					selectedIdsContent;
