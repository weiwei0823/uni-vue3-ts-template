<template>
	<view class="input-fixed-box">
		<view class="kuaisan-board" :style="{ height: uWindowHeight + 'px' }">
			...
			<view class="top-header" v-else>
				<nav-bar :nav-bar-height="'80rpx'" :isBack="false" :left-width="220" :right-width="220" :title="''"
					...
					<view class="board-content">
						<view class="board-content-item">
							<hash-info @goHash="toUrl({url: '/pages/liuhe/hashIntro'})" :hash-info="headInfo.hashInfo" :stampType="headInfo.stampType"
								:nextShorNo="headInfo.nextShorNo" :lottery-id="headInfo.lotteryId"
								v-if="headInfo.hashInfo"></hash-info>
							<view class="top" v-else>
								<text v-if="this.showRoundInfo.stampType === 3 || this.showRoundInfo.stampType === 1">{{
									`${$t("pages.liuhe.common.text1")}${showRoundInfo.nextShorNo ?
										showRoundInfo.nextShorNo : ''}${$t("pages.liuhe.common.text2")}` }}</text>
								<text v-else>{{ `${$t("pages.liuhe.common.text1")}${showRoundInfo.shorNo ?
									showRoundInfo.shorNo : ''}${$t("pages.liuhe.common.text2")}` }}</text>
							...
							<view class="ball-list">
								<view class="ball-list-item" v-if="headInfo.stampType === 2"
									...
									:key="index">
									<view class="ball-num" :style="setBallStyle(item)">
									</view>
								</view>
							</view>
						...
						<view class="board-content-item-right">
							<view class="info">
								...
								<view class="line2">
									{{ $t("pages.liuhe.common.text1") }}<text>{{ showRoundInfo.nextShorNo
										}}</text>{{ $t("pages.liuhe.common.text2") }}
								</view>
								<view class="line3">
									<text v-if="showRoundInfo.stampType === 1" style="color:#FFFF00">{{
										$t("pages.liuhe.common.text5") }}</text>
									<text v-if="showRoundInfo.stampType === 2">{{ $t("pages.liuhe.common.text6")
										}}</text>
									<text style="color: #FFFF00" v-if="showRoundInfo.stampType === 3">{{
										$t("pages.liuhe.common.text7") }}</text>
								</view>
			...
			<view :class="isShowRecords ? 'tab-content active' : 'tab-content'" :style="setTabStyle">
				<view :class="isShowOrder ? 'bet-list active' : 'bet-list'" v-show="current == 1">
					<betting-area ref="betting" @changlongOrder="changlongOrder" @setOrders="setOrders"
				...
				<view v-if="isLoadingChatRoom" v-show="current == 3" name="current-3" style="height: 100%;">
					<view v-if="!userInfo" class="chat-room-unlogin">
						<view @click="toLogin"><span style="color: blue">{{ $t("pages.liuhe.common.text8") }}</span>{{
							$t("pages.liuhe.common.text9") }}
						</view>
					</view>
...
import BettingArea from "./kuaisan/betting";
import Buy from "./common/buy";
import Records from "./kuaisan/records";
...
export default {
	...
	data() {
		return {
			...
			navBg: require('@/static/img/liuhe/kuaisan/nav.svg'),
	...
	async onLoad(option) {
		let _this = this;
		//缓存方式是错的 在浏览器上用可能会改变浏览器大小  暂时保留这种 新增监听变动
	...
	methods: {
		...
		getHeadInfo() {
			getRewardsInfo({
				...
				if (res.result.numberList === null) {
					res.result.numberList = [];
				}
		...
		setOrders(data) {
			...
			if (ball.code === "ksbthethdxth" || ball.code === "ksbthethdxbth" || ball.code === "ksbthethfx") {
				...
				if (ball.minNum === 1) {
					...
					this.isShowOrder = this.orderList[0].stakeCount > 0 ? true : false;
				}
				this.$refs.buy.getHistoryAmount();
				// this.methodName =this.orderList[0] ? this.orderList[0].methodName : '';
				let arr = [];
				if (!this.orderList.length) return null
				this.orderList.forEach(item => {
					item.ballChildren = ballChildren;
					if (item.content) {
		...
		formatOrderList(order) {
			this.orderList.forEach(item => {
				this.formatItem(item, order);
				this.calculateOrderTotal(item);
			});
		},

		formatItem(item, order) {
			if (item.dantuo) {
				this.formatDantuoItem(item);
			} else if (item.tonghaoSelected) {
				this.formatTonghaoItem(item);
			}
			this.setItemPrice(item, order);
		},

		formatDantuoItem(item) {
			const { dan, tuo } = item.dantuo;
			const danNames = this.findNameByIds([], dan);
			const tuoNames = this.findNameByIds([], tuo);
			const danIds = this.findIdsByDantuo([], dan);
			const tuoIds = this.findIdsByDantuo([],tuo);

			item.content = danIds.map(String).join(' ') + "," + tuoIds.map(String).join(' ');
			item.name = danNames.join(' ') + "," + tuoNames.join(' ')
			delete item.dantuo;
		},

		formatTonghaoItem(item) {
			const { tonghao, butonghao } = item.tonghaoSelected;
			const thNames = this.findNameByIds([], tonghao);
			const bthNames = this.findNameByIds([], butonghao);
			const thIds = this.findIdsByDantuo([], tonghao);
			const bthIds = this.findIdsByDantuo([], butonghao);

			item.content = thIds.map(String).join(' ') + "," + bthIds.map(String).join(' ');
			item.name = thNames.join(' ') + "," + bthNames.join(' ')
			delete item.tonghaoSelected;
		},

		setItemPrice(item, order) {
			item.unitPrice = Number(order.unitPrice) || 1;
			item.rebatePoint = Number(order.userRebate);
		...
		calculateOrderTotal(item) {
			...
			this.orderTotal.price += item.totalPrice;
		},

		filterOrderList() {
			this.$store.dispatch('setOrderList', this.orderList.filter(item => item.totalPrice !== 0));
		},

		navigateToCart() {
			const code = this.orderList[0]?.code;
			const id = this.lotteryId;
-			this.playing.setStorage(code, this.orderList);
			uni.navigateTo({
				url: `/pages/liuhe/cart?code=${code}&id=${id}`
			});
		},
// -		/**
// -		 * 打开底部下注弹窗
...
// -					//如果是胆拖，需要重新组装显示订单和下单的参数
// -					if (item.dantuo) {
// -						let danNames = this.findNameByIds([], item.dantuo.dan);
// -						let tuoNames = this.findNameByIds([], item.dantuo.tuo);
// -						let danIds = this.findIdsByDantuo([], item.dantuo.dan);
// -						let tuoIds = this.findIdsByDantuo([], item.dantuo.tuo);
// -
// -						item.content = danIds.map(String).join(' ') + "," + tuoIds.map(String).join(' ');
// -						item.name = danNames.join(' ') + "," + tuoNames.join(' ')
// -						delete item.dantuo; // true
// -					} else if (item.tonghaoSelected) {
// -						let thNames = this.findNameByIds([], item.tonghaoSelected.tonghao);
// -						let bthNames = this.findNameByIds([], item.tonghaoSelected.butonghao);
// -						let thIds = this.findIdsByDantuo([], item.tonghaoSelected.tonghao);
// -						let bthIds = this.findIdsByDantuo([], item.tonghaoSelected.butonghao);
// -
// -						item.content = thIds.map(String).join(' ') + "," + bthIds.map(String).join(' ');
// -						item.name = thNames.join(' ') + "," + bthNames.join(' ')
// -						delete item.tonghaoSelected; // true
// -					}
// -					if (item.unitPrice === 0) {
// -						item.unitPrice = 1
// -					}
...
// -				});
// -				if (this.menuList.find(item => item.code === 'cart')?.isOpen) { //角标4表示右上角菜单，打开了购物车功能
// -					let code = '';
// -					if (this.orderList[0]) {
...
// -					})
// -				} else {
// -					this.orderList = this.orderList.filter(item => item.totalPrice !== 0);
// -					this.$refs.confirm.open("center");
// -				}
// -			} else {
// -				uni.$emit('showPop', {
...
// -		},
		...
		closeBuy() {
			...
			this.$store.dispatch('setOrderList', []);
			if (this.cartList.length > 0) { //如果购物车不是空的，窗口不能被关闭
				return
			}
			this.isShowOrder = false;
		},
...
.kuaisan-board {
	...
	.top-header {
		...
		.board {
			background-image: url("@/static/img/liuhe/kuaisan/bg.svg");
			...
			.close-ball-list {
				...
				.close-ball-item {
					background-image: url("@/static/img/liuhe/kuaisan/close.svg");
					background-position: center;
			...
			.top {
				...
				font-size: 28rpx;
				text-align: center;
				padding: 35.5rpx 42.5rpx 39.5rpx 42.5rpx;
