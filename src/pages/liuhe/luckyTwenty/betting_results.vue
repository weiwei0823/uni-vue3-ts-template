			...
			<view class="confirm-box">
				...
				<view class="confirm-content">
					<view>
						...
						<view>第{{ itemDetail.lotteryNo }}期</view>
						<view>{{ itemDetail.playName }}{{ itemDetail.betValue }}</view>
						<view>投注{{ setAmount(itemDetail.betAmount) }}元</view>
					</view>
				</view>
      ...
      ...mapState(useAppInfoStore, ['themeColor'])
		...
		methods: {
			...
			onToNav(index) {
				let url = '';
				if (index === 1) {
					url = '/pages/liuhe/luckyTwenty?records=true' + '&id=' + this.lotteryId;
				} else {
					url = '/pages/liuhe/luckyTwenty?id=' + this.lotteryId
				}
				uni.redirectTo({
					url: url
				})
