<template>
	<view class="input-fixed-box">
		<view class="shishicai-board" :style="{height: uWindowHeight+'px'}">
			...
			<view class="top-header" v-else>
				<nav-bar :nav-bar-height="'80rpx'" :headerTop="keyBoardHeight" :isBack="false" :left-width="220"
					:right-width="220" :title="''" :titleStyle="titleStyle" :font-color="titleStyle.color"
					:header-style="{
          height: '40px',
          ...
          position: 'relative'
					...
					<block slot="right">
						<view class="nav-right">
							<view :class="refresh ? 'balance refresh' : 'balance'" @click="refreshBalance">
								...
								<text class="balance-text" style="color: #FFFFFF"
									v-else>¥&nbsp;{{ setAmount(0) }}</text>
					...
					<view class="board-content">
						<view class="board-content-item-left">
							...
							<view class="top" v-else>
								<text v-if="this.showRoundInfo.stampType === StampType.InTheEnvelope">{{ `${$t("pages.liuhe.shishicai.text1")}${showRoundInfo.nextShorNo ? showRoundInfo.nextShorNo : ''}${$t("pages.liuhe.shishicai.text2")}`
                  }}</text>
								<text
									v-else>{{ `${$t("pages.liuhe.shishicai.text1")}${showRoundInfo.shorNo ? showRoundInfo.shorNo : ''}${$t("pages.liuhe.shishicai.text2")}` }}</text>
								<text class="status" @click="toAccount">{{$t("pages.liuhe.shishicai.text3")}}</text>
							</view>
							<view class="ball-list">
								<view class="ball-list-item" v-if="headInfo.stampType === StampType.CutOff"
							...
							<view class="close-ball-list" v-if="headInfo.stampType === StampType.InTheEnvelope">
								<view class="close-ball-item" v-for="item in closeShowBall">
									<view class="ball-text">
										<image :src="require('./../../static/img/liuhe/questionMark.png')"></image>
									</view>
								</view>
							</view>
							<view class="result-detail" v-if="headInfo.stampType === StampType.CutOff">
						...
						<view class="board-content-item">
							<view class="info">
								<view class="line1" v-if="isLive" @click="isShowLive = true">
									...
									<text class="live-text">{{$t("pages.liuhe.shishicai.text4")}}</text>
								</view>
								<view class="line2">
									{{$t("pages.liuhe.shishicai.text1")}}<text>{{ showRoundInfo.nextShorNo }}</text>{{$t("pages.liuhe.shishicai.text2")}}
								</view>
								<view class="line3">
									<text
										v-if="showRoundInfo.stampType === StampType.WaitingForOpening">{{$t("pages.liuhe.shishicai.text5")}}</text>
									<text
										v-if="showRoundInfo.stampType === StampType.CutOff">{{$t("pages.liuhe.shishicai.text6")}}</text>
									<text style="color: #FFFF00"
										v-if="showRoundInfo.stampType === StampType.InTheEnvelope">{{$t("pages.liuhe.shishicai.text7")}}</text>
								</view>
				...
				<view v-if="isLoadingChatRoom" v-show="current == 3" style="height: 100%;">
					<view v-if="!userInfo" class="chat-room-unlogin">
						<view @click="toLogin"><span
								style="color: blue">{{$t("pages.liuhe.shishicai.text8")}}</span>{{$t("pages.liuhe.shishicai.text9")}}
						</view>
					</view>
			...
			<show-modal :title="modalTitle" ref="show" @submit="clearCartToBack">
				<view slot="content" class="modal-content">
					{{$t("pages.liuhe.shishicai.text11")}}

					<view>{{$t("pages.liuhe.shishicai.text12")}}</view>
				</view>
			</show-modal>
			<uni-popup :title="$t('pages.liuhe.shishicai.text13')" type="dialog" ref="result">
				<view class="error-modal">
					<view class="title">

						{{$t("pages.liuhe.shishicai.text14")}}
					</view>
					<view class="content">
						{{ errorMsg }}
					</view>
					<view class="footer" :style="{color:themeColor.themeColor}" @click="yes()">
						{{$t("pages.liuhe.shishicai.text15")}}
					</view>
				</view>
			</uni-popup>
...
<script>
	...
	import BettingArea from "./ssc/betting";
	import Buy from "./common/buy";
	import Records from "./ssc/records";
  ...
  import useWebsocketStore from "@/store/modules/websocketStore";
	...
	export default {
		...
		data() {
			return {
				StampType,
				modalTitle: this.$t("pages.liuhe.shishicai.text14"),
				isShowMiPai: false,
				errorMsg: "",
				yourBettingList: [],
				isShowLive: false,
				...
				navBg: require('@/static/img/liuhe/ssc/title.png'),
				...
				orders: [],
				tabList: [{
						label: this.$t("pages.liuhe.shishicai.text16"),
						value: 1,
					},
					{
						label: this.$t("pages.liuhe.shishicai.text17"),
						value: 2,
					},
					{
						label: this.$t("pages.liuhe.shishicai.text18"),
						value: 3,
					},
				],
				id: 0,
      ...
      if (option.down) {
        uni.showToast({
          title: this.$t("pages.liuhe.shishicai.text19"),
          icon: "success"
        })
      }
      if (option.cancel == 0) {
        this.current = 2;
        pop(this.$t("pages.liuhe.shishicai.text20"), 1000);
      }
      if (option.isLive == 1) {
        this.isLive = true;
      }
      ...
      if (isLogin()) {
		...
		methods: {
			...
			goBack() {
				if (this.cartList.length > 0) {
					uni.$emit('showPop', {
						refName: 'AlertMsg',
						data: {
							title: this.$t("pages.liuhe.shishicai.text22"),
							content: this.$t("pages.liuhe.shishicai.text23")
						},
						confirm: () => {
							this.cartList = [];
			...
			toUrl(item) {
				...
				// if (item.code === "chart") {
				// 	uni.showToast({
				// 		title: this.$t("pages.liuhe.common.text21"),
				// 		icon: "none"
				// 	})
				// 	return
				// }
				if (item.code === "recent") {
					item.url = "/pages/liuhe/ssc/recent_draws"
					uni.navigateTo({
			...
			getLotteryInfo(e) {
				...
				if (data.source === 2) {
					if (obj.lotteryId == this.lotteryId) {
						...
						if (this.headInfo.lotteryNo < obj.lotteryNo && mipaiObj.isOpen && this.headInfo.stampType !== StampType.InTheEnvelope) {
							// console.log("getLotteryInfo更新")
							this.getBettingList();
							if (this.isShowMiPai) {
			...
			getHeadInfo() {
				getRewardsInfo({
					...
					if (res.result.stampType === StampType.WaitingForOpening) {
          ...
          console.log("orderItem",item)
			...
			//前往购买
			toBuy(obj) {
				if (this.headInfo.stampType === StampType.WaitingForOpening) {
					this.isCloseBuy = true;
				}
				if (this.userInfo) {
...
<style lang="scss" scoped>
	...
	.shishicai-board {
		...
		.top-header {
			...
			.board {
				background-image: url("@/static/img/liuhe/ssc/header.png");
