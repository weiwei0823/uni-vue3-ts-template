<template>
  <view class="ball-list">
    <view v-for="(item, index) in ballChildren" :class="item.isSelected === true  ? 'ball-item selected' : 'ball-item'" :data-id="item.id" :key="item.id" @click="onClickItem(item)">
          <view class="box">
            <view class="left">
              <view class="num">
                <text>{{item.name}}</text>
  ...
  methods:{
    setDefaultSelected() {
      ...
      this.orderList.forEach(item=>{
          if(item.isMultiple){
            item.content.split(",").forEach((id)=>{
              id&&!isNaN(id)&&selectedIds.push(parseInt(id));
            })
          }else {
            selectedIds.push(item.playId);
          }
      })
