<template>
	<view class="input-fixed-box">
		<view class="luckyTwenty-board" :style="{height: uWindowHeight+'px'}">
          ...
          position: 'relative'
                    ...
                    ''}${$t("pages.liuhe.liuhePlaying.text2")}`
							...
							<view class="ball-list">
								<view class="ball-list-item" v-if="headInfo.stampType === 2"
									...
									:key="index">
									<view class="ball-num" :style="setBallBg(item)">
			...
			<order-comfirm v-if="haveChanglong === false" ref="orderClass" :title="title" :showRoundInfo="showRoundInfo"
				:orderList="orderList" :orderTotal="orderTotal" :isBetting="isBetting" @cancelBtn="cancelBtn"
				@toBuyOrder="toBuyOrder">

			</order-comfirm>
			<changlong-order-comfirm v-if="haveChanglong" ref="orderClass" :changlong="changlong"
...
<script>
	...
	import BettingArea from "./luckyTwenty/betting";
	import Buy from "./common/buy";
	import Records from "./luckyTwenty/records";
	...
	export default {
		...
		data() {
			return {
				...
				navBg: require('@/static/img/liuhe/lucky/header-bg.svg'),
				numbers: ["05", "45", "49", "39", "12", "40", "10"],
				isShowRecords: false,
				colorList: this.$config.getStorageSync(this.$config.enumMgr.APP_LOCALSTORE_KEYS.LOCAL_STORE_LOTTERYTYPE)
					.colorList,
      ...
      if (isLogin()) {
		...
		methods: {
			...
			toUrl(item) {
				...
				if (item.code === "recent") {
					item.url = "/pages/liuhe/luckyTwenty/recent_draws"
					uni.navigateTo({
			...
			cancelBtn() {
				this.$refs.orderClass.close("center");
			},
...
<style lang="scss" scoped>
	...
	.luckyTwenty-board {
		...
		.top-header {
			...
			.board {
				background-image: url("@/static/img/liuhe/lucky/board-bg.svg");
