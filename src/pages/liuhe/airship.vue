		...
		<view class="liuhe-board" :style="{ height: uWindowHeight + 'px' }">
				...
				<view class="board" :style="{ paddingTop: `${keyBoardHeight}px` }">
					...
					<view class="board-content">
						<view class="board-content-item">
							<hash-info @goHash="toUrl({ url: '/pages/liuhe/hashIntro' })" :hash-info="headInfo.hashInfo"
								:stampType="headInfo.stampType" :nextShorNo="headInfo.nextShorNo"
								:lottery-id="headInfo.lotteryId" v-if="headInfo.hashInfo"></hash-info>
							<view class="top" v-else>
								<text v-if="this.showRoundInfo.stampType === 3 || this.showRoundInfo.stampType === 1">{{
									`${$t("pages.liuhe.airship.text1")}${showRoundInfo.nextShorNo ?
										showRoundInfo.nextShorNo : ''}${$t("pages.liuhe.airship.text2")}` }}</text>
								<text v-else>{{ `${$t("pages.liuhe.airship.text1")}${showRoundInfo.shorNo ?
									showRoundInfo.shorNo : ''}${$t("pages.liuhe.airship.text2")}` }}</text>
								<text class="status" @click="toAccount">{{ $t("pages.liuhe.airship.text3") }}</text>
							</view>
							<view class="ball-list">
								<view class="ball-list-item" v-if="headInfo.stampType === 2 || headInfo.stampType === 1"
							...
							<!--              <view class="close-ball-list" v-if="headInfo.stampType === 1">-->
							<!--                <view class="close-ball-item" v-for="item in closeShowBall"></view>-->
							<!--              </view>-->
							<view class="result-detail" v-if="headInfo.stampType === 2 || headInfo.stampType === 1">
								<view class="result-detail-item" :key="index"
									v-for="(item, index) in handleResultDetail(headInfo.resultDetail)">
									{{ item }}
								</view>
							</view>
						</view>
						<view class="board-content-item-right">
							<view class="info">
								<view class="line1" v-if="isLive" @click="isShowLive = true">
									...
									<text class="live-text">{{ $t("pages.liuhe.airship.text4") }}</text>
								</view>
								<view class="line2">
									{{ $t("pages.liuhe.airship.text1") }}<text>{{ showRoundInfo.nextShorNo
										}}</text>{{ $t("pages.liuhe.airship.text2") }}
								</view>
								<view class="line3">
									<text v-if="showRoundInfo.stampType === 1"
										style="color:#FFFF00">{{ $t("pages.liuhe.airship.text5") }}</text>
									<text
										v-if="showRoundInfo.stampType === 2">{{ $t("pages.liuhe.airship.text6") }}</text>
									<text style="color: #FFFF00"
										v-if="showRoundInfo.stampType === 3">{{ $t("pages.liuhe.airship.text7") }}</text>
								</view>
			...
			<view :class="isShowRecords ? 'tab-content active' : 'tab-content'" :style="setTabStyle">
				<view :class="isShowOrder ? 'bet-list active' : 'bet-list'" v-show="current == 1">
					<betting-area ref="betting" @setOrders="setOrders" @selected="onTypeSelected"
						@changlongOrder="changlongOrder" :playList="playList" @onSelectedType="onSelectedType" />
				</view>
				<view v-if="current == 2" v-show="current == 2">
				...
				<view v-if="isLoadingChatRoom" v-show="current == 3" style="height: 100%;">
					<view v-if="!userInfo" class="chat-room-unlogin">
						<view @click="toLogin"><span
								style="color: blue">{{ $t("pages.liuhe.airship.text8") }}</span>{{ $t("pages.liuhe.airship.text9") }}
						</view>
					</view>
			...
			<show-modal :title="modalTitle" ref="show" @submit="clearCartToBack">
				<view slot="content" class="modal-content">
					{{ $t("pages.liuhe.airship.text11") }}
					<view>{{ $t("pages.liuhe.airship.text12") }}</view>
				</view>
			</show-modal>
			<uni-popup :title="$t('pages.liuhe.airship.text13')" type="dialog" ref="result">
				<view class="error-modal">
					<view class="title">
						{{ $t("pages.liuhe.airship.text14") }}
					</view>
					<view class="content">
						{{ errorMsg }}
					</view>
					<view class="footer" :style="{ color: themeColor.themeColor }" @click="yes()">
						{{ $t("pages.liuhe.airship.text15") }}
					</view>
				</view>
			</uni-popup>
...
import BettingArea from "./airship/betting";
import Buy from "./common/buy";
import Records from "./airship/records";
...
export default {
	...
	data() {
		return {
			modalTitle: this.$t("pages.liuhe.airship.text14"),
			isShowLive: false,
			dataList: [],
			isRefresh: true,
			title: "",
			...
			navBg: require('@/static/img/liuhe/airship/header-bg.svg'),
			...
			orders: [],
			tabList: [{
				label: this.$t("pages.liuhe.airship.text16"),
				value: 1,
			},
			{
				label: this.$t("pages.liuhe.airship.text17"),
				value: 2,
			},
			{
				label: this.$t("pages.liuhe.airship.text18"),
				value: 3,
			},
			],
			id: 0,
	...
	async onLoad(option) {
		...
		if (option.down) {
			uni.showToast({
				title: this.$t("pages.liuhe.airship.text19"),
				icon: "success"
			})
		}
		if (option.cancel == 0) {
			this.current = 2;
			pop(this.$t("pages.liuhe.airship.text20"), 1000);
		}
	...
	methods: {
		...
		async getBettingList() {
			const { code, result: { page: { records } } } = await list({
				type: 1,
				lotteryNo: this.headInfo.lotteryNo,
				lotteryId: this.lotteryId
			});
			if (code === 200) this.yourBettingList = records;
		},
		closeMiPai() {
		...
		toAccount() {
			...
			if ([1, 2, 3, 4].indexOf(parseInt(lotteryType)) == -1) {
				uni.navigateTo({
		...
		goBack() {
			...
			uni.$emit('showPop', {
				refName: 'AlertMsg',
				data: {
					title: this.$t("pages.liuhe.airship.text22"),
					content: this.$t("pages.liuhe.airship.text23"),
				},
				confirm: () => {
		...
		toUrl(item) {
			...
			if (item.code === "recent") {
				item.url = "/pages/liuhe/airship/recent_draws"
				uni.navigateTo({
		...
		getLotteryInfo(e) {
			...
			if (data.source === 2) {
				if (obj.lotteryId == this.lotteryId) {
					...
					if (this.headInfo.lotteryNo < obj.lotteryNo && mipaiObj.isOpen && this.headInfo.stampType !== 3) {
						this.getBettingList();
						if (this.isShowMiPai) {
		...
		setRewardsTimeOut() {
			...
			var sendCondRun = function (useSecend) {
				...
				if (time < 0 || time === 0) {
					_this.lists();
					// console.log("useSecend", useSecend)
		...
		doUpdateHeaderInfoFromSocket(content) {
			...
			if (this.headInfo.lotteryNo < content.lotteryNo && mipaiObj.isOpen && content.stampType !== 3) {
				// console.log(content, "headInfocontent");
				// console.log(this.headInfo, "headInfocontent");
...
.nav-bar:before {
	...
	width: 100%;
	height: 80rpx;
	background: linear-gradient(90deg, #2982ac 30%, #32639b);
	position: absolute;
	z-index: -1
}
...
.liuhe-board {
	...
	.top-header {
		...
		.board {
			background-image: url("@/static/img/liuhe/airship/bg.svg");
			...
			.close-ball-list {
				...
				.close-ball-item {
					background-image: url("@/static/img/liuhe/airship/close.svg");
