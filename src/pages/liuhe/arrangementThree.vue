<template>
	<view class="input-fixed-box">
		<view class="arrangement-three" :style="{height: uWindowHeight+'px'}">
			...
			<view class="top-header" v-else>
				<nav-bar :nav-bar-height="'80rpx'" :isBack="false" :left-width="220" :right-width="220" :title="''"
					:headerTop="keyBoardHeight" :titleStyle="titleStyle" :font-color="titleStyle.color"
					:header-style="setNavBg()" class="nav-bar">
					<block slot="left">
					...
					<block slot="right">
						<view class="nav-right">
							<view :class="refresh ? 'balance refresh' : 'balance'" @click="refreshBalance">
								...
								</text> <text class="balance-text" style="color: #FFFFFF"
				...
				<view class="board" :style="[setBoardStyle(),{paddingTop:`${keyBoardHeight}px`}]">
					...
					<view class="board-content">
						<view class="board-content-item-left">
							<view class="top">
								<text v-if="this.showRoundInfo.stampType === 3">{{ `${$t("pages.liuhe.common.text1")}${showRoundInfo.nextShorNo ? showRoundInfo.nextShorNo :
			...
			<show-modal :title="modalTitle" ref="isLogin" @submit="toLogin">
				<view slot="content" class="modal-content">
					...
					{{$t("pages.liuhe.common.text33")}}
					<view>{{$t("pages.liuhe.common.text34")}}</view>
				</view>
			</show-modal>
...
<script>
	...
	import BettingArea from "./arrangementThree/betting";
	import Buy from "./common/buy";
	import Records from "./arrangementThree/records";
	...
	export default {
		...
		data() {
			return {
				...
				navBg: require('@/static/img/liuhe/three/header-bg.png'),
				navBg3D: require('@/static/img/liuhe/three/header-3d.png'),
		...
		methods: {
          ...
          uni.$emit('showPop', {
			...
			toUrl(item) {
				...
				if (item.code === "recent") {
					item.url = "/pages/liuhe/arrangementThree/recent_draws"
					uni.navigateTo({
			...
			setNavBg() {
				if (this.lotteryId == 33) {
					return {
						height: "40px",
						backgroundImage: `url(${this.navBg3D})`,
			...
			setOrders(data) {
				...
				this.orderList.forEach(item => {
					if (item.content) {
						...
						this.isShowOrder = item.stakeCount >= 1 ? true : false;
				...
				if (this.menuList[5].isOpen) { //快速投注下单不跳转
					...
					buyOrder(param).then(res => {
						if (res.code === 200) {
							...
							this.$refs.confirm.close();
							if (this.$config.getStorageSync(this.$config.enumMgr.APP_LOCALSTORE_KEYS
									.LOCAL_STORE_USERINFO).userType == 'demo') {
								uni.$emit('showPop', {
									refName: 'AlertTestMsg',
								})
							} else {
