      ...
      <uni-row>
        ...
        <uni-col :span="12">
          <view class="right">
            ...
            <view class="num">
              <span-box :value="item.betMultiple" :ball-info="item" :ballPosInfo="{index: index, length: ballChildren.length}"></span-box>
            </view>
          </view>
        </uni-col>
      </uni-row>
...
export default {
  ...
  methods:{
    ...
    onClickItem(item){
      console.log("selecteds", this.currentArr, item)
      if(this.currentArr.length >= this.ball.maxNum && item.isSelected != true){
        uni.showToast({
