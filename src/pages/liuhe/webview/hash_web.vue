<template>
	<view class="fixed-locked" :class="['theme-' + themeColor.name]">
		...
		<view class="back-box-new-hash" v-if="isShow" @click="backConfrim" @touchstart="drag_start"
		...
		<view name="webview-box" :style="{ height: webViewHeight }">
			<web-view ref="webview" id="gameWebview" :style="{ height:webViewHeight }" v-if="isShow"
				:src="url"></web-view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			backHome() {
                uni.redirectTo({
					url: `/pages/liuhe/hashIntro?id=${this.lotteryId}&lotteryName=${this.lotteryName}`,
				});
			},
			out() {
				let prams = {
