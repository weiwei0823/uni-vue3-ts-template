<template>
	<view class="betting">
			...
			<view class="left-nav">
				<scroll-view scroll-y="true" class="list" :style="setNavLeftStyle()">
					<view :class="item.isDefault === 1 ? 'list-item selected' : 'list-item'"
						...
						<view class="type-name" :data-lockfont="item.name&&item.name.length>=4?1:0">
							{{ item.name }}
						</view>
					</view>
				</scroll-view>
		...
		<view class="right">
			...
			<play ref="play" :missOrMatch="missOrMatch" :play-data="playData" @selectedBall="setOrders"
